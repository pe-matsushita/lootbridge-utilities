...
describe('pool math', () => {
  it('should compute collateral balance from blockchain data', () => {
    ...
    // reserve: YFI
    const scaledLBTokenBalance = '161316503206059870';
    ...
    const expectedLBTokenBalance = '161594727054623229';
    const underlyingBalance = getLinearBalance({
      balance: scaledLBTokenBalance,
      index: liquidityIndex,
      rate: currentLiquidityRate,
      lastUpdateTimestamp,
      currentTimestamp,
    }).toString();
    expect(underlyingBalance).toEqual(expectedLBTokenBalance);
  });
